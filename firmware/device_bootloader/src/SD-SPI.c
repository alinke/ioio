...
//    printf("SD-SPI -- MDD_SDSPI_InitIO  TRISC: %04x  ODCC: %04x  LATC: %04x\n", TRISC, ODCC, LATC);
...
#endif
{
    ...
    //    printf("OpenSPIM  TRISD: %04x ODCD: %04x  TRISC: %04x ODCC: %04x  SPI1STAT: %04x  SPI1CON1 = %04x  _SDI1R: %d  MOSI_RP: %d  _RP25R: %d\n", TRISD, ODCD, TRISC, ODCC, SPI1STAT, SPI1CON1, _SDI1R, MOSI_RP, _RP25R);
}
...
MEDIA_INFORMATION *  MDD_SDSPI_MediaInitialize(void)
{
    ...
    do
    {
        ...
        //Send CMD0 to software reset the device
        response = SendMediaSlowCmd(GO_IDLE_STATE, 0x0);
	//	printf("timeout: %d   response.r1_byte = %02x\n", timeout, response.r1._byte);
    }while((response.r1._byte != 0x01) && (timeout != 0));
    ...
    if(timeout == 0)
    {
      //      printf("Media failed CMD0 too many times. R1 response byte = %02x\n", ((unsigned char*)&response + 1));
	  ...
	  //      printf("Media still failed CMD0. Cannot initialize card, returning.\n");
            #ifdef __DEBUG_UART  
        ...
        {
	  //	  printf("Media successfully processed CMD0 after CMD12.\n");
    ...
    {
      //      printf("Media successfully processed CMD0.\n");
        #ifdef __DEBUG_UART  
    ...
    if(((response.r7.bytewise.argument._returnVal & 0xFFF) == 0x1AA) && (!response.r7.bitwise.bits.ILLEGAL_CMD))
   	{
		...
		if(timeout >= 0xFFFF)
		{
		  //		  printf("Media Timeout on CMD55/ACMD41.\n");
            #ifdef __DEBUG_UART  
    ...
    {
      //      printf("Timeout occurred while processing CMD9 to read CSD register.\n");
